...
def load_data(database_filepath: str)->Tuple[pd.DataFrame, pd.DataFrame, List[str]]:
    ...
    Arguments: database_filepath: the path of the database
    Returns:    X: messages/ Features
                y: one-hot encoded/ categories
                categories list in order
    '''
    ...
    # Making DataFrame with needed categories
...
def tokenize(text:str)->List[str] :
    ...
    return [lemmatizzer.lemmatize(x).lower().strip() for x in tokens]
...
def evaluate_model(model: GridSearchCV, X_test: pd.DataFrame, Y_test: pd.DataFrame, category_names: List)->None:
    ...
    Arguments:   Model, features, labels to evaluate, & categories
    Returns: Report of Classification
    '''
